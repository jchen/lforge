...
pred coordInit[v: CoordinatorHost] {
    ...
    ParticipantHost /*as ParticipantHost → Prop*/ . v.votes = NoneVote
...
pred ptcpInit[v: ParticipantHost] {
    ...
    v.preference in Yes /* as Set Vote */ -- + No /* as Set Vote */
...
pred DistributedSystemInit[d: DistributedSystem] {
    ...
    -- TODO: Fix things of the form all x : expr where expr is not a type!
    all p: ParticipantHost | {    -- all participants also in start state
        p in d.participants implies
        ptcpInit[p]
    }
}
...
pred coordSendReq[v0: CoordinatorHost, v1: CoordinatorHost, send: Message, recv: Message] {
    --coordWellformed[v0]
    -- ??
    send = VoteReqMsg /* as Set VoteReqMsg */
    recv = NoneMessage
    v0 = v1
}
...
pred successfulRun {
    ...
    {no ds: DistributedSystem | TwoPC.nextState[ds] = s0} and
    {DistributedSystemInit[s0]} and
    --pred DistributedSystemNext[d0: DistributedSystem, d1: DistributedSystem,
                        --    step: Steps, send: Message, recv: Message, phost: ParticipantHost,
                        --    decision: Decision]
    ...
    {DistributedSystemNext[s0, s1,
                          CoordSendReqStep, VoteReqMsg, NoneMessage, none, NoneDecision]} and
